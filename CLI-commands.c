...
/*
 * Implements the shoutcast command.
 */
static BaseType_t prvShoutcastStartChannelCommand(char *pcWriteBuffer, size_t xWriteBufferLen,
...
/* Structure that defines the "echo_3_parameters" command line command.  This
 ...
 time. */
static const CLI_Command_Definition_t xShoutcast =
{ "shoutcast", "shoutcast <radio-url>:\r\n Starts playing the given Shoutcast radio\r\n\r\n",
		prvShoutcastStartChannelCommand, /* The function to run. */
...
void vRegisterCLICommands(void)
{
	...
	FreeRTOS_CLIRegisterCommand(&xShoutcast);
													...
													const char *pcCommandString)
{
	...
	url = FreeRTOS_CLIGetParameter(pcCommandString, /* The command string itself. */
	...
	configASSERT(url);
	if (xQueueSend(radioChannelQueue, url, 0) == pdPASS)
	{
		snprintf(pcWriteBuffer, xWriteBufferLen, "Starting radio: %s\r\n", url);
	}
	else
	{
		snprintf(pcWriteBuffer, xWriteBufferLen, "Failed to start radio: %s\r\n", url);
	}
